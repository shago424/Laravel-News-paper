    ...
    <nav class="navbar navbar-inverse" role="navigation">
      ...
      <div id="navbar" class="navbar-collapse collapse">
        <ul class="nav navbar-nav main_nav">
          ...
          @php
                $subcats = App\SubCategory::where('category_id',$category->id)->where('status',1)->get();
          @endphp
          <li class="dropdown"> <a href="{{ route('category.post',$category->slug) }}" class="dropdown-toggle" data-toggle="dropdown" role="button" aria-expanded="false">{{ $category->name }}</a>
            <ul class="dropdown-menu" role="menu">
              @foreach($subcats as $subcat)
              <li><a href="{{ route('subcategory.post',$subcat->slug) }}">{{ $subcat->name }}</a></li>
              @endforeach
            </ul>
          </li>
          @endforeach
