        ...
        <div class="left_content">
          <div class="single_page">
            <ol class="breadcrumb" style="height: 50px;background-color: #1A5276;color: #fff">
              <li style="padding-top:;font-size: 18px;padding-top: 5px"><a href="{{ route('public') }}">ক্যাটেগরি : প্রাইভেট চাকুরী  </a></li>
              
              
            </ol>
           
            @if($allpjposts->isEmpty())
            ...
            <div class="related_post">
              <div class="table-responsive">
                  <table class="table table-responsive table-hover" style="border:solid 1px">
                      ...
                      <tbody>
                        @foreach($allpjposts as $key => $pj)
                          <tr>
                              <td style="text-align: center;">{{ $key+1 }}</td>
                              <td style="text-align: center;">  <img src="{{ asset('upload/postimage/'.$pj->image) }}" width="50px" height="50px"></td>
                              <td width="15%" style="text-align: center;">{{ date('d-m-Y',strtotime($pj->created_at)) }}</td>
                              <td class="media-body"><a class="catg_title" style="color: blue" href="{{ route('singlepost',$pj->slug) }}">{{ $pj->title }}</a> </td>
                              <td class="media-body"><a style="color: blue" class="catg_title" href="{{ route('singlepost',$pj->slug) }}">বিস্তারিত</a></td>
                              @if($pj->file ==NULL)
                              <td></td>
                              @else
                              <td style="text-align: center;"> <a  target="_blank" href="{{ asset('upload/postfile/'.$pj->file) }}"><i class="fa fa-download" style="width: 50px;color:blue"></a></td>
                              @endif
                          </tr>
                          @endforeach
                      </tbody>
